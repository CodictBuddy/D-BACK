...
let ConnectionsController = class ConnectionsController {
    constructor(connService, sservice) {
        this.connService = connService;
        this.sservice = sservice;
    }
    async getDetail(res, token, organization, body) {
        try {
    ...
    async remove(res, token, organization, user_id, connection_type) {
        try {
    ...
    async list(res, token, organization, view_type, connection_status, connection_type) {
        try {
            let data = await this.connService.getConnectionList(+organization, token, connection_status, connection_type, +view_type);
            return res.json(data);
        }
        catch (err) {
            const { code, response } = this.sservice.processError(err, this.constructor.name);
...
], ConnectionsController.prototype, "getDetail", null);
__decorate([
    common_1.UseGuards(jwt_auth_guard_1.JwtAuthGuard),
    common_1.Post(''),
    __param(0, common_1.Res()),
...
__decorate([
    common_1.UseGuards(jwt_auth_guard_1.JwtAuthGuard),
    common_1.Delete(':user_id/:connection_type'),
    __param(0, common_1.Res()),
    __param(1, getuser_decorator_1.GetToken()),
    __param(2, common_1.Param('organization_code')),
    __param(3, common_1.Param('user_id')),
    __param(4, common_1.Param('connection_type')),
    __metadata("design:type", Function),
...
__decorate([
    common_1.UseGuards(jwt_auth_guard_1.JwtAuthGuard),
    common_1.Get(':view_type/:connection_type/:connection_status'),
    __param(0, common_1.Res()),
    __param(1, getuser_decorator_1.GetToken()),
    __param(2, common_1.Param('organization_code')),
    __param(3, common_1.Param('view_type')),
    __param(4, common_1.Param('connection_status')),
    __param(5, common_1.Param('connection_type')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, usertoken_dto_1.UserToken, Object, Object, Object, Object]),
    __metadata("design:returntype", Promise)
], ConnectionsController.prototype, "list", null);
ConnectionsController = __decorate([
    common_1.Controller(':organization_code/connection'),
